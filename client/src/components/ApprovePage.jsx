...
export default function DashPosts() {
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setPendingPosts(data.posts.filter(post => !post.approved));
          setApprovedPosts(data.posts.filter(post => post.approved));
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(
        `/api/post/approve/${postIdToDelete}`,
        {
          method: 'PUT',
        }
      );
      ...
      if (res.ok) {
        ...
        setApprovePost("Post Approve Sucessfully");
        const approvedPost = pendingPosts.find(post => post._id === postIdToDelete);
        setPendingPosts(prev => prev.filter(post => post._id !== postIdToDelete));
